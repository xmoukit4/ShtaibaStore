	...
	$res=mysqli_query($con,"select `order`.*,users.name from `order`,users where `order`.txnid='$payment_request_id' and `order`.user_id=users.id");
		if(mysqli_num_rows($res)>0){
			...
			if($payment_status=='Credit'){
				...
				mysqli_query($con,"update `order` set payment_status='complete', payid='$payment_id' where txnid='$payment_request_id'");
				sentInvoice($con,$oid);
				?>
				<script>
				...
				mysqli_query($con,"update `order` set payment_status='fail', payid='$payment_id' where txnid='$payment_request_id'");
				?>
				<script>
